...
// ONE UNAVOIDABLE LINE, required to compile at all in this project: `Omit<...,
// "onSelect">`, not the plain interface upstream extends. React 19.2 /
// @types/react 19.2.18 give native <button> its own `onSelect` DOM event prop,
// colliding with this component's own `onSelect: (id: string) => void` -
// upstream's original `extends ButtonHTMLAttributes<HTMLButtonElement>` fails
// with TS2430 here. Type-only; zero runtime, class-name or geometry change.
...
export interface NotchNavProps extends HTMLAttributes<HTMLDivElement> {
  ...
  // SECOND AND THIRD UNAVOIDABLE ADDITIONS, alongside the `NotchItemProps`
  // type fix documented at this file's top - both purely additive, both a
  // no-op unless a consumer passes them:
  //
  //   - `contentViewportRef`: used as the page's real shell, the
  //     "Scrollable Content Viewport" div (below) becomes the ONLY thing
  //     that scrolls - `window` never does. Anything downstream keyed to
  //     real window scroll (framer's `useScroll` with no `container`) needs
  //     a ref to that div to track instead.
  //   - `contentClassName`: upstream's viewport is `flex ... items-center
  //     justify-center`, sized for ONE centred demo card. A real page's
  //     children are stacked sections taller than the viewport, so a
  //     consumer needs to override that to a top-aligned column - `!`
  //     important is required since Tailwind's utility output order isn't
  //     guaranteed to put a later class after an earlier one of the same
  //     specificity.
