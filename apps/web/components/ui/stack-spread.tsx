...
//
// VENDORED THIRD-PARTY COMPONENT - `hyperiux/stack-spread` from 21st.dev, MIT
// licensed, installed 2026-09-18 from the paid registry with Praneeth's API key.
// Treat this file as upstream source: keep edits minimal and listed here so a
// future re-install can be diffed rather than guessed at.
//
// DELIBERATE EDITS (only these):
//   1. `StackSpreadStage` is exported. Upstream it is private and the default
//      export hardcodes its own eight demo images and headline, which makes the
//      component unusable with real content. Exporting the stage is the
//      smallest change that lets this project pass its own cards in.
//   2. `headline` / `subtitle` props added to the stage, replacing the
//      hardcoded "Design That Responds." copy and the `SUB` constant.
//   3. Defaults added to one `useTransform` destructure, for this project's
//      `noUncheckedIndexedAccess`. Types only, identical behaviour.
//   4. This header.
//   5. `scrollContainer` prop added to the stage, forwarded to `useScroll`'s
//      `container` option (upstream leaves it unset, which tracks `window`).
//      Needed because `SiteHeader` renders this project's actual page shell
//      as `NotchNav`, which makes `NotchNav`'s own internal viewport the
//      thing that scrolls - not `window` - so `wrapRef`'s position against
//      `window` scroll never changes and the scatter would never animate.
//      `ProofStackSpread` supplies that viewport via `useScrollContainer()`.
//      Unset, this is upstream's original window-tracking behaviour.
// Nothing else is touched: the scatter mechanism, the spring parallax, the
// responsive behaviour and the reduced-motion handling are upstream's.
//
// `<img>` rather than `next/image` is upstream's choice. Left alone rather than
// rewritten, so the file stays diffable; the eslint rule is disabled for this
// file only, for that reason.
...
function Card({
  ...
  const translate = useTransform(
    ...
    // Defaults on the destructure: upstream types this callback's argument as
    // `number[]`, which under this project's `noUncheckedIndexedAccess` makes
    // every element possibly-undefined. `= 0` is the identity value for all
    // three here (no progress, no pointer offset), so this changes types only,
    // never behaviour.
    ([p = 0, px = 0, py = 0]: number[]) => {
...
export interface StackSpreadStageProps {
  ...
  /** centre headline; replaces upstream's hardcoded demo copy */
  ...
  /** scroll container to track instead of `window` - see edit 5 above */
...
export default function StackSpread({
  ...
  return (
    <StackSpreadStage
      cards={CARDS}
      // Upstream's own demo copy, kept here so the default export still behaves
      // exactly as shipped now that the stage takes its text as props.
      headline={
        <>
